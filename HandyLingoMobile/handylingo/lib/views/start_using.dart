...
class _StartUsingPageState extends State<StartUsingPage> with WidgetsBindingObserver {
  final String _serverUrl = "http://192.168.1.6:8000/predict"; 
  ...
  Future<void> _startCaptureSequence() async {
    ...
    } catch (e) { setState(() => _currentStatus = "Error"); }
  }
  ...
  Future<void> _uploadFrames(List<XFile> frames) async {
    ...
    } catch (e) { setState(() => _currentStatus = "Retry..."); }
    finally {
  ...
  void _initializeSignWeb() {
    _signWebController = WebViewController()..setJavaScriptMode(JavaScriptMode.unrestricted)..loadRequest(Uri.parse('https://sign.mt'));
  }
        ...
        children: [
          TextButton(
            ...
            child: Column(mainAxisSize: MainAxisSize.min, children: [
              Text(_mode == InputMode.signLanguage ? '3D' : 'SL', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 16, color: Colors.blue)),
              const Text('Switch', style: TextStyle(fontSize: 9, color: Colors.grey)),
            ]),
          ),
          Text('HANDYLINGO', style: GoogleFonts.inter(fontWeight: FontWeight.w700, fontSize: 15)),
          IconButton(icon: const Icon(Icons.person, size: 24), onPressed: () => Navigator.push(context, MaterialPageRoute(builder: (_) => const AccountPage()))),
        ],
      ),
    );
  }
        ...
        children: [
          Expanded(
            ...
            child: Container(
              ...
              decoration: BoxDecoration(color: Colors.black, borderRadius: _mode == InputMode.signLanguage ? BorderRadius.circular(20) : BorderRadius.zero),
                  ...
                  ? (_cameraController?.value.isInitialized ?? false 
                      ? Stack(fit: StackFit.expand, children: [
                          ...
                          Positioned(
                            top: 10, left: 10,
                            child: ChoiceChip(
                              label: Text(_languageType == SignLanguageType.asl ? "ASL" : "FSL"),
                              selected: true,
                              onSelected: (val) => setState(() => _languageType = _languageType == SignLanguageType.asl ? SignLanguageType.fsl : SignLanguageType.asl),
                            ),
                          ),
                          
                          if (_isCapturing) Center(child: Text("$_capturedCount/10", style: const TextStyle(color: Colors.white, fontSize: 48, fontWeight: FontWeight.bold))),
                        ])
              ...
              child: Column(
                ...
                children: [
                  if (_mode == InputMode.signLanguage) ...[
                    ...
                    Text("MODE: ${_languageType.name.toUpperCase()} - $_currentStatus", style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue, fontSize: 10)),
                    Expanded(child: SingleChildScrollView(child: Text(_accumulatedSentence.isEmpty ? "Perform your sign..." : _accumulatedSentence, style: const TextStyle(fontSize: 20, fontWeight: FontWeight.bold), textAlign: TextAlign.center))),
                    Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
                      ElevatedButton.icon(
                        onPressed: (_isCapturing || _isSending) ? null : _startCaptureSequence,
                        icon: const Icon(Icons.videocam, size: 20),
                        label: Text(_isCapturing ? "RECORDING" : "CAPTURE SIGN", style: const TextStyle(fontSize: 12)),
                        style: ElevatedButton.styleFrom(backgroundColor: _isCapturing ? Colors.red : Colors.green, foregroundColor: Colors.white, padding: const EdgeInsets.symmetric(horizontal: 20, vertical: 10)),
                      ),
                      IconButton(onPressed: () => setState(() => _accumulatedSentence = ""), icon: const Icon(Icons.refresh, color: Colors.blue)),
                    ]),
                  ] else const Spacer(),

...
class _StartUsingPageState extends State<StartUsingPage> with WidgetsBindingObserver {
  ...
  Future<void> _initCamera() async {
    ...
    // Select camera based on _isFrontCamera
  ...
  void _toggleCamera() {
    if (_isCapturing || _isSending) return; // Prevent switching during capture
    setState(() {
  ...
  Future<void> _uploadFrames(List<XFile> frames) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        setState(() {
          String word = json['prediction_label'] ?? "Unknown";
          if (word != "(none)") {
            _accumulatedSentence += (_accumulatedSentence.isEmpty ? "" : " ") + word.toUpperCase();
          }
              ...
              child: Column(
                children: [
                  if (_mode == InputMode.signLanguage) ...[
                    ...
                    Text(_currentStatus.toUpperCase(), style: const TextStyle(fontWeight: FontWeight.bold, color: Colors.blue, fontSize: 10)),
                    const SizedBox(height: 5),
                    Expanded(
                      child: SingleChildScrollView(
                        child: Text(
                          _accumulatedSentence.isEmpty ? "Perform your sign..." : _accumulatedSentence, 
